...
exports.purchaseOrder = catchAsync(async (req, res, next) => {
	...
	await Cart.update({status: "purchased"}, { where: { userId: currentUser.id, id: cart.id} })
	...
	await Promise.all(
		cart.productsInCarts.map(async (product) => {
			const productFind = await Product.findOne({ where: { id: product.productId}})
			return await productFind.update({ quantity: productFind.quantity - product.quantity})
		})
	)
	...
	const order = {
		orderProducts,
		orderOfUser: orderUser
	}

	res.status(202).json({message: 'exito', data: order})
...
exports.getOrderById = catchAsync(async (req, res, next) => {
	...
	const order = await Order.findOne({ where: { id: orderId, userId: currentUser.id}})
	// Must include the products of that order
	const productsOrder = await ProductInOrder.findOne({ where: { orderId: order.id}})
	// Must get the total price of the order and the prices of the products and how much the user bought

	const orderOfUser = {
		informationOrder: order,
		informationProductsOrden: productsOrder
	}
	
	res.status(200).json({ status: 'success', data: {orderOfUser} })
})
